...
$sql = mysqli_query($link,"SELECT * FROM `user` WHERE `poisk` > 0 and `poisk` != '".$poisk."' and `poisk` != '4'  and `sex` = 'Женский' LIMIT 1");
...
if ($b1[0] != 0) {
    ...
    $link->query("UPDATE `user` SET `poisk`='3' WHERE user_id = '".$user_id."'");
    $kbd = [
        'one_time' => false,
        'buttons' => [
